...
private:
  ...
  std::complex<float> random_complex_float(); // returns complex z such that modulus(z) < 1 and imaginary_part(z) > 0

  _FT exact_number_from_float(float x);
  _Cmplx exact_complex_from_float_complex(const std::complex<float>& z);

  bool try_to_compute_inexact_z0_from_z1_z2_z3(std::complex<float>& z0, std::complex<float>& z1, std::complex<float>& z2, std::complex<float>& z3);
...
template<class Traits>
std::complex<float> Hyperbolic_fundamental_domain_factory_2<Traits>::random_complex_float(){
  std::complex<float> result (random_float(), random_positive_float());
  while (std::norm(result) >= 1){
...
template<class Traits>
typename Traits::Complex Hyperbolic_fundamental_domain_factory_2<Traits>::exact_complex_from_float_complex(const std::complex<float>& z){
...
template<class Traits>
bool Hyperbolic_fundamental_domain_factory_2<Traits>::try_to_compute_inexact_z0_from_z1_z2_z3(std::complex<float>& z0, std::complex<float>& z1, std::complex<float>& z2, std::complex<float>& z3){
  if (   ((z2/z1).imag()<=0) || ((z3/z2).imag()<=0)   ){
    return false;
  }

  std::complex<float> one (1,0);
  std::complex<float> u = (one - z1*std::conj(z2))   *   (one - z2* std::conj(z3));
  float a = -(u* std::conj(z1)*z3).imag();
  float b = (u*(z3- std::conj(z1))).imag();
  float c = u.imag();
...
bool Hyperbolic_fundamental_domain_factory_2<Traits>::try_to_compute_exact_z3_from_z0_z1_z2(_Cmplx& z0, _Cmplx& z1, _Cmplx& z2, _Cmplx& z3){
  ...
  if ( (std::norm(z0)>=one_number) || (std::norm(z1)>=one_number) || (std::norm(z2)>=one_number) || (std::norm(z3)>=one_number) ){
    return false;
  }
  ...
  _Cmplx intermediate = (one_cmplx - f_of_z0* std::conj(f_of_z1)) * (one_cmplx - f_of_z1* std::conj(f_of_z2));
  _FT P_of_zero = intermediate.imag();
  _FT P_of_one = (intermediate * (one_cmplx-f_of_z2* std::conj(f_of_z3))).imag();
  ...
  if ( (V.imag()<=zero_number) || (std::norm(V)>=one_number) || ((V/f_of_z2).imag()<=zero_number) ){
    return false;
  }
...
bool Hyperbolic_fundamental_domain_factory_2<Traits>::sanity_check(_Cmplx& z0, _Cmplx& z1, _Cmplx& z2, _Cmplx& z3){
  ...
  _Cmplx Z = (one_cmplx-z0* std::conj(z1)) * (one_cmplx-z1* std::conj(z2)) *(one_cmplx-z2* std::conj(z3)) *(one_cmplx+z3* std::conj(z0));
