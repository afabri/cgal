...
LindstromTurkCore(const Params& aParams, const Profile& aProfile)
  :
    mParams(aParams),
    mProfile(aProfile),mConstraints_n(0),
...
extract_boundary_data()
{
  ...
  for(const_border_edge_iterator it=mProfile.border_edges().begin(),
      eit=mProfile.border_edges().end(); it!=eit; ++it)
  {
...
extract_triangle_data()
{
  ...
  for(const_triangle_iterator it=mProfile.triangles().begin(),
      eit=mProfile.triangles().end(); it!=eit; ++it)
  {
                                                 ...
                                                 const Triangle_data_vector& aTriangles)
{
  ...
  // Volume optimization
  for(typename Triangle_data_vector::const_iterator it=aTriangles.begin(),
      eit=aTriangles.end(); it!=eit; ++it)
  {
    const Triangle_data& lTri = *it;
...
compute_shape_cost(const Point& p, const vertex_descriptor_vector& aLink)
{
  ...
  for(typename vertex_descriptor_vector::const_iterator it = aLink.begin(),
      eit=aLink.end(); it!=eit; ++it)
...
add_constraint_from_gradient(const Matrix& H, const Vector& c)
{
  ...
  switch(mConstraints_n)
  {
    ...
    case 1:
    {
      ...
      Vector AbsA0(CGAL::abs(A0.x()), CGAL::abs(A0.y()), CGAL::abs(A0.z()));
      Vector Q0;
