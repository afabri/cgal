...
struct My_visitor : SMS::Edge_collapse_visitor_base<Surface_mesh>
{
  ...
  void OnCollapsing(const Profile&, boost::optional<Point> placement)
  {
    if(!placement)
  ...
  void OnCollapsed(const Profile&, Vertex_handle)
  {
    ++stats->collapsed;
  }
...
int main(int argc, char** argv)
{
  ...
  for(Surface_mesh::Halfedge_iterator eb = surface_mesh.halfedges_begin(),
                                      ee = surface_mesh.halfedges_end(); eb!=ee; ++eb)
    eb->id() = index++;

  index = 0;
  for(Surface_mesh::Vertex_iterator vb = surface_mesh.vertices_begin(),
                                    ve = surface_mesh.vertices_end(); vb!=ve; ++vb)
    vb->id() = index++;
