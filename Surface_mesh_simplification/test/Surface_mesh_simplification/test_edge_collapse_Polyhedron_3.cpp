...
class Link_builder
  : public CGAL::Modifier_base<Surface::HalfedgeDS>
{
...
protected:
  ...
  void add_triangle(Builder& aBuilder, const Profile::Triangle& aTri)
  {
...
template<class T>
ostream& operator<<(ostream& os, const boost::optional<T>& o) { return os << opt2str(o); }
      ...
      if(lSurface.is_valid())
      {
        if(lSurface.is_pure_triangle())
        {
          ...
          if(sUseMP)
          {
            ...
            edge_collapse(lSurface, stop, CGAL::parameters::get_cost(cost)
          ...
          cerr << "\r" << aName << (rSucceeded ? " succeeded" : "FAILED")
               << ". Now " << (lSurface.size_of_halfedges() / 2) << " edges"
               << ". Elapsed time=" << t.time() << " seconds." << endl;
