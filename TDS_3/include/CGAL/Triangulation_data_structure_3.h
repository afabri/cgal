...
>
using Triangulation_data_structure_3_storage_type = Indexed_storage<Vb, Cb, Concurrency_tag_>;
...
public:
  ...
  typedef Iterator_range<Cell_iterator>    Cell_handles;
  typedef Iterator_range<Vertex_iterator>  Vertex_handles;
...
public:
  ...
  void print_cells(std::ostream& os,
                   const Unique_hash_map<Vertex_handle, size_type, std::hash<Vertex_handle> > &V ) const;
...
public:
                               ...
                               Vertex_handle newv)
  {
      for (CellIt cit = cell_begin; cit != cell_end; ++cit)
          tds().tds_data(*cit).mark_in_conflict();
...
public:
  ...
  {
    CGAL_precondition( dimension() == 2 );
    return Face_circulator(&tds(), v, cell(v));
  }
...
public:
  ...
  static std::array<Vertex_handle, 2> vertices(const Edge& e)
  {
    return std::array<Vertex_handle, 2>{
             vertex(e.first, e.second),
             vertex(e.first, e.third)};
  }
  static std::array<Vertex_handle, 3> vertices(const Facet& f)
  {
    return std::array<Vertex_handle, 3>{
             vertex(f.first, vertex_triple_index(f.second, 0)),
             vertex(f.first, vertex_triple_index(f.second, 1)),
             vertex(f.first, vertex_triple_index(f.second, 2))};
  }
  /*  AF won't work for pure index
                        ...
                        int prev_ind2, int depth)
{
    ...
    CGAL_precondition( dimension() == 3);
    CGAL_precondition( tds().tds_data(c).is_in_conflict() );
    CGAL_precondition( ! tds().tds_data(neighbor(c,li)).is_in_conflict() );
    ...
    for (int ii=0; ii<4; ++ii) {
      ...
      // turn around the oriented edge vj1 vj2
      while ( tds().tds_data(n).is_in_conflict() ) {
        CGAL_assertion( n != c );
        cur = n;
      ...
      // Now n is outside region, cur is inside.
      tds().tds_data(n).clear(); // Reset the flag for boundary cells.
...
create_star_2(Vertex_handle v, Cell_handle c, int li )
{
  ...
  do {
    ...
    while ( tds().tds_data(neighbor(cur, cw(i1))).is_in_conflict() ) {
      // neighbor in conflict
      cur = neighbor(cur, cw(i1));
      i1 = index( cur, v1 );
    }
    tds().tds_data(neighbor(cur,cw(i1))).clear();
  ...
  // j has no meaning if false is returned
{
  CGAL_precondition( dimension() == 3 );
  assert(false);
  return true ;  // AF: todo ( has_vertex(c,v,j) && (j != i) );
}
  ...
  // checks whether the query facet (c,i) has vertex v
{
  ...
  int j;
  assert(false);
  return true;  // AF: todo( c->has_vertex(v,j) && (j != i) );
}
...
void
Triangulation_data_structure_3<Vb,Cb,Ct,St>::
print_cells(std::ostream& os, const Unique_hash_map<Vertex_handle, size_type, std::hash<Vertex_handle>> &V ) const
{
  Unique_hash_map<Cell_handle, size_type, std::hash<Cell_handle> > C(0, number_of_cells());
  size_type i = 0;
  ...
  case 3:
    {
      ...
      do {
          Cell_handle cc = *ccir;
          cells.push_back(cc);
          tds().tds_data(cc).mark_in_conflict();
          ++ccir;
      } while (c != *ccir);
...
decrease_dimension(Cell_handle c, int i)
{
  ...
  Vertex_handle w = vertex(c,i); //  AF:  why not w = v ?
  // the cells incident to w are down graded one dimension
  // the other cells are deleted
  std::vector<Cell_handle> to_delete, to_downgrade;
...
{
  bool v_is_valid = is_valid(v, verbose,level);
  bool has_vertex = has_vertex(cell(v), v);
  bool v_is_vertex = vertices().is_used(v);
  bool vertex_cell_is_cell = cells().is_used(cell(v));
    ...
    case 3:
      {
        int i;
        for(i = 0; i < 4; i++) {
          if ( vertex(c,i) == Vertex_handle() || vertices().is_used(vertex(c,i)) == false ) {
            if (verbose)
        ...
        const ConvertCell& convert_cell)
{
  ...
  // Initializes maps
  Unique_hash_map< typename TDS_src::Vertex_handle,Vertex_handle, std::hash<typename TDS_src::Vertex_handle> > V(Vertex_handle(), tds.number_of_vertices());
  Unique_hash_map< typename TDS_src::Cell_handle,Cell_handle, std::hash<typename TDS_src::Cell_handle>  > F(Cell_handle(), tds.number_of_cells());
  ...
  for (typename TDS_src::Cell_iterator cit = tds.cells().begin();
      ...
      F[*cit]=ch;
      for (int j = 0; j < dim; j++)
        ch->set_vertex(j, V[vertex(*cit, j)] );
      convert_cell(*cit,*ch);
  }
